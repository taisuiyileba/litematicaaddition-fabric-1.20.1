...
public class LitematicaSchematic
{
    ...
    private static LitematicaSchematic INSTANCE;
    private static File sInitializationFile;
    private static FileType sInitializationType;
    ...
    public LitematicaSchematic(@Nullable File file, FileType schematicType)
    {
        this.schematicFile = file;
        this.schematicType = schematicType;
        this.converter = SchematicConverter.createForLitematica();
    }
    public static void configure(@Nullable File file , FileType schematicType) {
        if (INSTANCE != null) {
            throw new IllegalStateException("单例已初始化，配置必须在首次获取前设置");
        }
        sInitializationFile = file;
        sInitializationType = schematicType;
    }
    public static LitematicaSchematic getInstance() {
        if (INSTANCE == null) {
            synchronized (LitematicaSchematic.class) {
                if (INSTANCE == null) {
                    if (sInitializationFile == null && sInitializationType == null) {
                        throw new IllegalStateException("必须先调用configure()设置初始化参数");
                    }
                    INSTANCE = new LitematicaSchematic(sInitializationFile, sInitializationType);
                }
            }
        }
        return INSTANCE;
